...
1. `cd $WT && git add -A && git commit -m wip`; `git status --porcelain` must be empty. If the user refuses, `git stash -u` into `$REPO` and stop.
2. `git ls-files '.env*' '**/.env*'`: templates only, suffix `example|sample|template|dist|defaults`. Keep the rest in a secret manager or outside the repo; copy into the new cwd. Gitignored `node_modules`, `target`, `dist` stay; Shade rebuilds deps from the lock as shared layers.
...
4. `shade open $REPO --base $BR --session <id>`; `$BR` may be unpushed, it resolves from `$REPO`. Export its `SHADE_*` env, cd to its `cwd`. No claude/codex/cursor/kimi/zumith ancestor: add `--owner-pid $$` or run `shade heartbeat`.
