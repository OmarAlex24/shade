...
4. `shade open $REPO --base $BR --session <id>`; export its `SHADE_*` env, cd to its `cwd`. No claude/codex/cursor/kimi/zumith ancestor: add `--owner-pid $$` or run `shade heartbeat`.
